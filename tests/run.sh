...
echo "== Perfiles =="
ARCA_PERFIL=survive ARCA_EXTRAS=""
recurso_activo survive && ! recurso_activo core && ! recurso_activo full && ok "survive incluye solo survive" || fail "survive"
ARCA_PERFIL=15y; perfil_valido 15y && [[ $(perfil_normalizar 15y) == survive && $(perfil_rango 15y) == 1 ]] && ok "15y es alias de survive" || fail "alias 15y"
ARCA_PERFIL=core ARCA_EXTRAS=""
recurso_activo survive && recurso_activo core && ! recurso_activo recovery && ! recurso_activo full && ok "core incluye survive y core" || fail "core"
ARCA_PERFIL=recovery
recurso_activo survive && recurso_activo core && recurso_activo recovery && ! recurso_activo full && ok "recovery incluye survive, core y recovery" || fail "recovery"
ARCA_PERFIL=full
...
ARCA_PERFIL=15y; perfil_cargar; [[ $ARCA_PERFIL == survive ]] && ok "perfil_cargar normaliza 15y" || fail "normalizar en perfil_cargar" "$ARCA_PERFIL"
...
n=$(packs_read "$PACKS_CONF" | wc -l); [[ $n == 6 ]] && ok "packs_read: 6 líneas activas (comentarios y vacías fuera)" || fail "packs_read" "$n"
...
ARCA_PERFIL=survive; n=0; while IFS=$'\t' read -r p _ _ _ _; do recurso_activo "$p" && n=$((n + 1)); done < <(packs_read "$PACKS_CONF"); [[ $n == 1 ]] && ok "survive selecciona 1 de 6 ZIM" || fail "selección survive" "$n"
ARCA_PERFIL=core; n=0; while IFS=$'\t' read -r p _ _ _ _; do recurso_activo "$p" && n=$((n + 1)); done < <(packs_read "$PACKS_CONF"); [[ $n == 3 ]] && ok "core selecciona 3 de 6 ZIM (incluye survive)" || fail "selección core" "$n"
ARCA_PERFIL=full; n=0; while IFS=$'\t' read -r p _ _ _ _; do recurso_activo "$p" && n=$((n + 1)); done < <(packs_read "$PACKS_CONF"); [[ $n == 5 ]] && ok "full selecciona 5 de 6 ZIM (sin extra)" || fail "selección full" "$n"
...
ARCA_PERFIL=survive; soft_activo llm && soft_activo printkit && ! soft_activo llm7b && ok "SOFT_PERFILES: survive lleva LLM 3B y print-kit, no el 7B" || fail "SOFT_PERFILES survive"
...
if [[ $(id -u) -eq 0 ]]; then
  ...
  grep -q 'P0 vivir 15 años' "$TMP/dry" && grep -q 'Recursos más pesados' "$TMP/dry" && ok "dry-run muestra prioridades y recursos pesados" || fail "dry-run resumen"
