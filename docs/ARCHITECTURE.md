...
`perfil` es el mínimo que incluye el recurso (`core` ⊂ `recovery` ⊂ `full`) o `extra:<nombre>`
(solo con `--extra`). `SOFT_PERFILES` en `software.conf` hace lo mismo para los bloques de
software. El perfil y los extras elegidos se guardan en `.arca/profile` y `.arca/extras`; todos
los scripts los leen con `perfil_cargar`. Las líneas sin atributos valen `full P2 general`.

La prioridad decide qué se protege con PAR2 (P0 pequeño), qué va en `bootstrap/` y cómo responde
el asistente; la categoría decide la carpeta, el LEEME y el modo del asistente (SOURCE_ONLY en
`medicina`, `agua`, `industria`).
